...
Разрабатывался и обкатан на NAV 2018; целевой сервер — та же платформа. Номер платформы
пакет установки нигде не зашивает — см. [Статус](#статус).
...
не включает вызванную функцию — сводит дерево колонка полного времени. У строки объекта
полное время — это время входов в него вместе со всем, что он вызвал: ушедшее в другие
объекты видно прямо в его строке, а вызовы внутри объекта в неё не задваиваются.
Сортировка либо по времени, либо в порядке исходника.
...
дважды. Подписчики событий открываются со строки издателя. У обычной процедуры, которая
стоит под своим объектом со временем, тело раскрывается щелчком по её заголовку —
повторный щелчок его закрывает.
...
**Встроенная обкатка.** Запуск `Codeunit 110202` (при снятой галке в настройке ничего
...
Живёт она в **отдельном объекте и ставить её не обязательно** — см.
[Обкатку можно не ставить](#обкатку-можно-не-ставить).
...
Дерево раскрывается от объекта к его функциям — и к триггерам, и к обычным
процедурам, в которые вошли прямым вызовом, — а дальше к отдельным операторам C/AL.
Строки кода корень раскрывает сразу только у точек входа; тело обычной процедуры
открывается щелчком по её заголовку.

**Обычные функции с верхнего уровня можно убрать, и по умолчанию они убраны.** Они не
двери: платформа их не зовёт, в них входят вызовом из своего же объекта, — а их время
вдобавок уже сидит внутри полного времени той двери, из которой их позвали, поэтому
колонка «полное» на верхнем уровне не складывается. Складывается только «своё»: сумма
по функциям сходится со строкой объекта до третьего знака. Галка «Показывать обычные
функции» (умолчание — в настройке) возвращает их на экран; без неё остаются одни точки
входа, а до обычной функции доводит провал со строки вызова. Отступает скрытие там, где
у объекта не отработала ни одна точка входа: под ним иначе не осталось бы ничего. И не
действует вовсе ниже подписи «Остальные объекты»: там свод, а не разбор, — туда смотрят,
чтобы понять, куда ушло время мимо корня, а объект с миллисекундами и пустотой под ним
на это не отвечает. Галка остаётся за корнем — тем, который и разбирают подробно.

**Корень дерева задаётся объектом, а замер при этом идёт по всем.** В режиме «Все
объекты» поля объекта заполняются по желанию: названный объект встаёт первым, каким бы
малым ни было его собственное время, а остальные уходят под подпись «Остальные объекты»
по убыванию полного времени. Так смотрят окно: измерено всё, что оно подняло, включая
штатные объекты платформы, но начинается список с него. У подписи раздела намеренно
пусто полное время — складывать его по объектам нельзя, взаимные вызовы легли бы дважды.
Корень переставляется на месте, без пересборки: кнопка «Сделать корнем» берёт объект
текущей строки, и загруженный замер это переживает так же, как только что снятый.
...
| Что | Подробности |
|-----|-------------|
| Платформа | NAV 2018 (build 11.x) — и на стенде, и на целевом сервере. Номер платформы пакет не зашивает: корень NAV берётся из пути службы |
...
| Конфиг инстанции | `EnableFullALFunctionTracing = true`. Без него событие оператора не шлётся вовсе и замер выходит пустым. Правится в `CustomSettings.config`, то есть требует **перезапуска инстанции** |
...
Ставятся две вещи: **17 объектов C/SIDE** (`LineProfiler.txt`) и **сборка приёмника**
...
### Обкатку можно не ставить

Из семнадцати объектов один — `Codeunit 110202 AL Profiler Self Test` — **инструменту
не нужен**. В нём живут пятьдесят пять проверок, которыми инструмент проверяет сам себя:
две тысячи строк, которые в боевой базе не выполняются никогда. Не импортировав его,
вы получите работающий профайлер целиком — страницы, замеры, сравнение, советник,
подсказки и демонстрационный замер, — просто без возможности прогнать самопроверку.

Обратной зависимости нет: главный кодюнит про обкатку не знает ни строкой, ссылок на
неё нет ни у одной страницы, и `Codeunit 110200` компилируется без неё. Зависимость
односторонняя — обкатка знает про инструмент, инструмент про обкатку нет.

Ставить её стоит там, где инструмент дорабатывают: она ловит отказы, которые с виду
выглядят исправной работой — потерянное время, стрелку, ведущую в никуда, замер,
который сохранился неполным. На объекте, где профайлером только пользуются, она
нужна разве что один раз, чтобы убедиться в исправности установки, — и для этого
хватает демонстрационного замера, который живёт в основном кодюните.
...
Компилировать надо **все 17 объектов**: импорт сбрасывает флаг `Compiled`, а на
недокомпилированной таблице сервер отвечает «Объект метаданных Table N не найден»
и в лог ничего не пишет.

Рядом лежит `LineProfiler.fob` — те же 17 объектов, выгруженные C/SIDE в двоичном
...
Контейнер и текст сейчас сходятся — проверяется прогоном:
...
```
35923af1e3735c96f6a14b9be8a2eb2d516ba8bd02833018e7d2e84f2e1984af *LineProfiler.fob
```
...
Работает и обкатан на стенде **NAV 2018**: встроенная обкатка даёт **56 из 56**,
все 17 объектов компилируются, демонстрационный замер проходит целиком. Прогон был
на приёмнике, собранном против TraceEvent 1.0.39 — версии из поставки NAV 2018. Ровно она
стоит и на целевом сервере, так что основной вариант в пакете — его вариант.
...
Что **не** проверено: сама установка на целевом сервере — её и снимает первая половина
пакета `onsite/`, шаг за шагом и со скриншотом на каждом; калибровка порогов подсказок на
большой живой бизнес-логике.
...
Пятая — встроенная обкатка (**56 из 56**) — запускается в самой базе NAV: то, что
...
|--------------|------------|
| `LineProfiler.txt` | 17 объектов C/SIDE одним файлом: `Table 110200..110207`, `Page 110200..110204`, `Report 110200`, `Codeunit 110200..110202`. UTF-8 без BOM, CRLF |
