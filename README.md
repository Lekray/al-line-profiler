...
Под объектом видно всё, что он отработал: сумма своего времени его функций сходится
с его собственной строкой, и время не может спрятаться за нераскрытой строкой. Строки
кода корень раскрывает сразу только у точек входа; тело обычной процедуры открывается
щелчком по её заголовку.
...
```
22c85006de9a2496726edb342b21623ad1b7be9e625382213d9132f3059cc212 *LineProfiler.fob
```
...
Работает и обкатан на стенде **NAV 2018**: встроенная обкатка даёт **52 из 52**,
...
Пятая — встроенная обкатка (**52 из 52**) — запускается в самой базе NAV: то, что
