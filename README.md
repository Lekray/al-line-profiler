...
Снят он с базы, в которую перед выгрузкой импортирован ровно `LineProfiler.txt` из
этого репозитория, так что расходиться этим двум файлам неоткуда; выгружала
платформа `11.0.36462.0`.
...
Ни CI, ни автоматических тестов в репозитории нет — проверка одна, встроенная обкатка,
и запускается она в самой базе NAV.
...
ни трассировки, ни дампа исходников не требуется. Эталон — **25 из 25**. Проверка
