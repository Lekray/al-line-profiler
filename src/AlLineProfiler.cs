...
namespace LineProfiler
{
    ...
    public sealed class AlLineProfilerReceiver
    {
        ...
        private void RecordEdge(SessionState st, Frame parent, Frame callee, double subtree)
        {
            ...
            // виток уже содержит целиком. Считаем полное время ребра по ВНЕШНЕМУ витку -
            // ровно как и полное время строки: иначе одно из двух чисел на экране
            // складывало бы витки, а другое нет. Вызовы и своё время идут все: вызов был
            // настоящий, а своё время витков не пересекается.
            bool nested = false;
