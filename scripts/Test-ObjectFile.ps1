...
.DESCRIPTION
    Проверяет один .txt с объектами задачи (классический текстовый экспорт C/SIDE):
      ...
      (i) длина строк: рукописное - код и комментарии - не длиннее 130 знаков.
          То, что C/SIDE пишет сам и всегда одной строкой (заголовок функции со
          всеми параметрами, объявления TextConst и DotNet, двуязычные свойства,
          описания полей и контролов), перенести нельзя - импорт падает, - поэтому
          оно считается отдельно и печатается числом, а в ошибки не идёт.
...
# --- (i) Длина строк ----------------------------------------------------------
# Правило проекта: строка не длиннее 130 знаков. Оно про то, что пишут РУКАМИ, -
# про код и комментарии. Остальное в этом файле пишет сам C/SIDE и всегда одной
# строкой; перенести такую строку нельзя, импорт падает на первой же. Поэтому они
# не нарушение, а отдельный счёт: если однажды длинной окажется рукописная строка,
# она встанет в ошибки, а не растворится среди двухсот чужих.
$maxLineLen = 130
$cSideOwn = @(
    '^\s*(LOCAL\s+)?PROCEDURE\b',                                 # заголовок функции с параметрами
    '\s:\s+(TextConst|DotNet)\b',                                 # объявление константы или сборки
    '^\s*(ENU|RUS)=',                                             # вторая половина двуязычного свойства
    '^\s*\w+ML=\[',                                               # CaptionML, ToolTipML, OptionCaptionML
    '^\s*(OptionString|Description|CalcFormula|TableRelation|Permissions)=',
    '^\s*\{'                                                      # описание поля или контрола
)
$genLong  = 0
$handLong = 0
for ($i = 0; $i -lt $lines.Count; $i++) {
    $l = $lines[$i]
    if ($l.Length -le $maxLineLen) { continue }
    $own = $false
    foreach ($p in $cSideOwn) { if ($l -match $p) { $own = $true; break } }
    if ($own) { $genLong++; continue }
    $handLong++
    $head = $l.Trim()
    if ($head.Length -gt 60) { $head = $head.Substring(0, 60) + '...' }
    AddError ($i + 1) ("строка длиннее {0} знаков ({1}): {2}" -f $maxLineLen, $l.Length, $head)
}
...
Write-Host ("Строк >{0}:  {1} рукописных, {2} от C/SIDE" -f $maxLineLen, $handLong, $genLong)
Write-Host ""
