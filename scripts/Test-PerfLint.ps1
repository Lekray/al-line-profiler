...
$cu = @(
    ...
    '  Cust.GET(No);'                                     # 17
    'ГраницаFOR()'                                        # 18
    '  FOR i := 1 TO Cust.COUNT DO BEGIN'                 # 19  граница - ОДИН раз
    '    Total := Total + i;'                             # 20
    '  END;'                                              # 21
    'УсловиеWHILE()'                                      # 22
    '  WHILE Buffer.FIND(''-'') DO BEGIN'                 # 23  условие - КАЖДУЮ итерацию
    '    Total := Total + 1;'                             # 24
    '  END;'                                              # 25
)
...
$idx = @(
    ...
    (@('5','Codeunit','50000','Проверка правил','25','620','1','19.08.26','12:00:00','TEST','0') -join $TAB)
...
# ---------------------------------------------------------------------------
# 8. Из обзора: заголовок цикла - не тело. В 'FOR i := 1 TO Cust.COUNT DO'
#    граница вычисляется ОДИН раз, до первой итерации, и запрос в ней не N+1;
#    прежде правило 112 уверенно советовало «вынести из цикла» то, что уже вне
#    цикла. У WHILE наоборот: условие проверяется каждую итерацию, и находка там
#    верна. Однострочный FOR исключения не получает - тело стоит на той же
#    строке, по строке их не различить, и пропустить настоящий N+1 хуже.
# ---------------------------------------------------------------------------
$m8a = Get-Msg $rowsCu 19 '112'
Test-Value 'граница FOR: находки нет'      ''    $m8a
$m8b = Get-Msg $rowsCu 23 '112'
Test-Value 'условие WHILE: находка есть'   $true ($m8b.Length -gt 0)
Test-Value 'условие WHILE: названо циклом' $true ($m8b -match 'в цикле')
$m8c = Get-Msg $rowsCu 9 '111'
Test-Value 'тело FOR: находка на месте'    $true ($m8c.Length -gt 0)
