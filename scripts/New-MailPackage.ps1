...
# Имя файла едет ВМЕСТЕ с нагрузкой, отдельной латинской строкой: в потоке base64 имени
# нет вовсе - там одни байты, - и без этой строки распаковщик называет собранное по
# тому, на что оно похоже. Латиница обязательна: объект русского имени не прочитает.
$payloadName = 'LineProfilerPackage.zip'
...
$bootLines = @(
    ...
    'а имя файла берёт из самой рассылки. Ни одного пути и ни одного имени файла',
    'внутри объекта не зашито.',
    '',
    'ЧТО СДЕЛАТЬ',
    ...
    '  3. Тела писем с частями сохранить НА СВОЕЙ машине - куда угодно, имена любые.',
    '  4. Object Designer -> Codeunit 110207 -> Run. Объект попросит файлы по одному:',
    '     указывать сохранённые части, а когда они кончатся - нажать Отмену.',
    '     Порядок значения не имеет: каждая часть несёт свой номер.',
    ('  5. Объект вернёт собранное диалогом сохранения, предложив имя ' + $payloadName + '.'),
    '     Сборка приёмника лежит внутри него, в каталоге receiver.',
    ('  6. Распаковать ' + $payloadName + ' Проводником, дальше по README.md.'),
    '  7. Codeunit 110207 после этого можно удалить - или оставить: он собирает любой',
    '     файл, а не только этот пакет, и пригодится для любой следующей присылки.',
    '',
    ...
    '',
    'На сервер класть ничего не нужно: объект спрашивает файлы у вас и вам же отдаёт',
    'результат. На сервере собранный файл живёт ровно до того мгновения, когда дошёл до',
    'вашей машины, и тут же убирается. Отменённое сохранение - единственный случай,',
    'когда файл на сервере остаётся, и объект прямо пишет, где именно он лежит.'
)
