...
function New-Header {
    ...
    # Номер части ЛАТИНИЦЕЙ и отдельной строкой. Заголовок выше написан по-русски, а
    # распаковщик на C/AL - сплошная латиница (он едет телом письма и сохраняется в
    # какой попало кодировке), поэтому свой номер он оттуда прочитать не может. Без
    # номера части склеиваются в порядке, в каком их выбрали мышью, - а base64,
    # склеенный не в том порядке, остаётся ВАЛИДНЫМ base64: раскодируется молча и
    # даёт мусор. Строка начинается с '#', меткой нагрузки не является и старым
    # распаковщиком просто пропускается.
    $h += ('#PART {0:d2} OF {1:d2}' -f $No, $Of)
    $h += '<<< НАЧАЛО ЧАСТИ ' + $No + ' >>>'
    return $h
}
...
#
# Даты и теги изменений НЕ переписываются датой сборки, хотя раньше переписывались.
# Тег изменения по соглашению проекта несёт дату САМОЙ правки, а подстановка гнала
# все теги на сегодня и стирала историю объекта; Date= в OBJECT-PROPERTIES при этом
# заявлял правку, которой в этот день не было. Рассылка объект не меняет - значит и
# датировать её собой не должна.
...
$bootLines = @(
    ...
    'NAV: он сам соберёт файл из частей 1..N. Сам объект пришёл ОТДЕЛЬНЫМ письмом',
    '(ЧАСТЬ 0B) - его тело нельзя ничем дополнять, иначе C/SIDE не примет файл.',
    '',
    'Объект универсален: он собирает ЛЮБОЙ файл, а не только архив, и принимает как',
    'наши письма с метками, так и чистый base64 без всякой разметки. Части он',
    'складывает по номерам, которые несёт каждая, - порядок выбора значения не имеет.',
    '',
    'ЧТО СДЕЛАТЬ',
    ...
    '  4. Object Designer -> Codeunit 110207 -> Run. Указывать части по очереди, на',
    '     вопрос о следующей нажать Отмену. Порядок значения не имеет.',
    '  5. Объект сохранит на ВАШУ машину один файл - LineProfilerPayload.zip.',
    '     Сборка приёмника лежит внутри него, в каталоге receiver.',
    '  6. Распаковать LineProfilerPayload.zip Проводником, дальше по README.md.',
    '  7. Codeunit 110207 после этого можно удалить - или оставить: он пригодится',
    '     для любой следующей присылки, чем бы она ни была.',
    '',
    ...
    '',
    'Если что-то не так - объект СКАЖЕТ, что именно, и ничего не сделает. Он не падает',
    'и не оставляет следов на сервере: ни одной ошибки в нём нет вовсе, любой отказ',
    'закрывает файлы, чистит временный каталог и выводит причину одним сообщением.',
    '',
