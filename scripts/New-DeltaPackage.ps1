...
$ch += ''
# С -Only часть объектов В ПАКЕТ НЕ ВОШЛА, хотя по хэшу изменилась. Писать про них
# "не изменились" - врать принимающей стороне.
$ch += $(if ($Only) {
    'Остальные объекты поставки (' + ($cur.Count - $take.Count) + ') в эту загрузку не входят.'
} else {
    'Не изменились и не вошли: ' + ($cur.Count - $take.Count) + ' объектов.'
})
if ($removed.Count -gt 0) {
    $ch += ''
