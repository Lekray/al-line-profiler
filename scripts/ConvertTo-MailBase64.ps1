...
.PARAMETER Zip
    Завернуть файл в архив ПЕРЕД кодированием. Распаковщику на C/AL это не нужно - он
    собирает любой файл, а имя приезжает отдельной строкой #NAME. Архив нужен для
    другого: он короче (письмо меньше) и в него влезает не один файл, а сколько угодно.
...
.EXAMPLE
    ...
    То же, но нагрузка сжата: письмо короче, а внутрь архива влезет и не один файл.
...
$tmpZip = $null
if ($Zip) {
    # Заворачиваем ради размера и ради того, чтобы в одно письмо влезло больше одного
    # файла. Сигнатуру архива распаковщик не требует, а имя доедет и без него.
...
for ($p = 0; $p -lt $total; $p++) {
    ...
    # Про сборку в NAV пишем ВСЕГДА, а не только для архива: распаковщик собирает любой
    # файл и тип определяет сам по сигнатуре.
    $head += @(
        ''
        'КАК СОБРАТЬ В NAV: импортировать Codeunit 110207, запустить, указать части по'
        'очереди, на вопрос о следующей части нажать Отмену. Объект отдаст файл на вашу'
        'машину и назовёт его по типу нагрузки.'
    )
    $head += @(
        ''
    ...
    # Имя файла - тоже отдельной латинской строкой: в потоке base64 имени нет, и без
    # этой строки распаковщик назовёт собранное по тому, на что оно похоже. Русское имя
    # он не примет - у него в алфавите имён одна латиница, - поэтому такое не шлём вовсе
    # и говорим об этом вслух, вместо того чтобы отправить строку, которую отвергнут.
    # При -Zip имя берётся у ИСХОДНОГО файла с расширением архива, а не у временного:
    # получателю нужно имя того, что он в итоге распакует, а не нашей промежуточной
    # склейки.
    $wanted = if ($Zip) { $file.BaseName + '.zip' } else { $file.Name }
    if ($wanted -match '^[A-Za-z0-9._ -]+$') {
        $head += ('#NAME ' + $wanted)
    } elseif ($p -eq 0) {
        Write-Host ('  имя ' + $wanted + ' не латиницей - в письмо не идёт, файл назовут по содержимому')
    }
    $head += @(
        ''
    )
