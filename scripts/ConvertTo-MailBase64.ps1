...
.PARAMETER Zip
    Завернуть файл в архив ПЕРЕД кодированием. Распаковщику на C/AL это больше не
    нужно - он собирает любой файл, - но архив даёт две вещи, которых голый поток не
    даёт: он короче (письмо меньше) и он несёт ИМЯ файла. Имя из самой нагрузки не
    восстановить: распаковщик узнаёт по сигнатуре только тип и кладёт результат как
    LineProfilerPayload.<тип>.
...
.EXAMPLE
    ...
    Письмо под распаковщик Codeunit 110207: в архиве доедет и имя файла.
...
$tmpZip = $null
if ($Zip) {
    # Заворачиваем ради имени и размера, а не ради распаковщика: сигнатуру архива он
    # больше не требует. Имя внутри архива - единственное место, где оно переживёт
    # дорогу: в самом потоке base64 имени нет.
...
for ($p = 0; $p -lt $total; $p++) {
    ...
    if ($Zip) {
        $head += @(
            ...
            'Он тут ради имени: в самом потоке base64 имени файла нет, а из архива оно'
            'достаётся вместе с содержимым.'
        )
    }
